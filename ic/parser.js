...
module.exports = new Parser(function analyseEC(parsedUrl, ec) {
  ...
  if ((match = /^\/deliver\/connect\/(([a-z]+)\/([0-9]{4})([0-9]{3}[0-9x])\/v([0-9]+)n([0-9]+)\/[a-z0-9]+)\.(pdf|html)$/i.exec(path)) !== null) {
    ...
    // https://www.ingentaconnect.com:443/content/imp/jcs/2018/00000025/f0020005
    result.rtype            = 'TOC';
    result.mime             = 'HTML';
    result.unitid           = match[1];
    result.title_id         = match[2];
  ...
  } else if ((match = /^\/contentone\/([a-z]+\/([a-z]+)\/([0-9]{4})\/([0-9]+)\/([a-z0-9]+)\/([a-z0-9_.-]+))$/i.exec(path)) !== null) {
  ...
  } else if ((match = /^\/content\/([a-z0-9]+\/([a-z]+)\/([0-9]{4})\/([0-9]+)\/([a-z0-9]+)\/([a-z0-9_.-]+))$/i.exec(path)) !== null) {
